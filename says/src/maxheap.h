...
public:
	...
	ty_tuple	*insertItem(const ty_tuple newData);			// heap-inserts newData, returns the address of it
	ty_tuple	popMaximum();							// removes and returns heap max, reheapifies
	ty_tuple	returnMaximum();						// returns the heap max; no change to heap
	void		printHeap();							// displays contents of the heap
	void		printHeapTop10();						// displays top 10 entries in the heap
	void		updateItem(ty_tuple *address, ty_tuple newData);   // updates the value of the tuple at address
	void		updateItem(ty_tuple *address, double newStored);// update only the stored value of tuple at address
	void		deleteItem(ty_tuple *address);				// remove an item from the heap
	int		returnArraysize();						// 
	int		returnHeaplimit();						// 
...
// Pop Maximum ------------------------------------------------------------------------
ty_tuple maxheap::popMaximum() {				// O(log k) time
	ty_tuple temp = returnMaximum();
	deleteItem(A[1].d);
	return temp;
}

// Return Maximum --------------------------------------------------------------------
ty_tuple maxheap::returnMaximum() {			// O(1) time
	ty_tuple temp;
	temp.m = A[1].d->m;					// 
	temp.i = A[1].d->i;					// 
	temp.j = A[1].d->j;					// 
...
ty_tuple* maxheap::insertItem(const ty_tuple newData) {			// O(log k) time
	int index;
	ty_tuple *pointer;
...
void maxheap::updateItem(ty_tuple *address, ty_tuple newData) {   // O(log k) time
...
void maxheap::updateItem(ty_tuple *address, double newStored) {	// O(log k) time
	double oldm    = address->m;
...
void maxheap::deleteItem(ty_tuple *address) {
	ty_tuple *swap;
