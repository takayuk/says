...
public:
	...
	ty_tuple	*heap_ptr;			// pointer to element's location in vektor max-heap
...
public:
	...
	ty_tuple	returnMaxKey();							// returns the maximum key in the tree
	ty_tuple	returnMaxStored();							// returns a tuple of the maximum (key, .stored)
	int		nodecount();							// returns number of items in tree
...
ty_tuple vektor::returnMaxStored() { return heap->returnMaximum(); }

ty_tuple vektor::returnMaxKey() {
	ty_tuple themax;
	element *current;
	current = root;
...
int vektor::nodecount() { return support; }
...
void vektor::deleteItem(int killKey) {
	...
	if (z != NULL) {
		ty_tuple newmax    = heap->returnMaximum();		// get old maximum in O(1)
...
void vektor::printTree() {
	ty_tuple max = heap->returnMaximum();
